...
const Listings = (props) => {
    ...
    const [link_facebook, setLinkFacebookPage ] = useState("");
    const [link_twitter, setLinkTwitter ] = useState("");
    const [link_instagram, setLinkInstagram ] = useState("https://www.instagram.com");
    const [link_linkedin, setLinkLinkedIn ] = useState("");
    const [link_youtube, setLinkYoutube ] = useState("");

    // merchant_id, merchant_name, merchant_about, themeColor should be fetched from db
    const merchant_name = "merchant_name"
    const merchant_about = "merchant_about"
    const theme_color = "#FFD9E6"

    const [product_name, setProductName ] = useState("LONG")
    const [product_description, setProductDesc ] = useState("product_description")

    const [picture_id, setPictureId ] = useState("")
    const [place_id, setPlaceId ] = useState("")
    const [about_host, setAboutHost] = useState("about_host")

    const [pricing_options, setPricingOptions ] = useState([]);
    const pricing_option_test = [
        {
          "price_type": "one_time",
          "amt": 3500,
    ...
    const {fetchAllProductsPending,skuData} = props;
    return (
    <>
    <Navigation/>
    <HamburgerMenu/>
    ...
    <div class="row pb-250">
        ...
        <button class="open_checkout color-white checkout-modal color-white btn bg-action-1 mx-15 md color-white radius8 px-20 f-20 text-adaptive product-btn" style={{backgroundColor:props.themeColor}}>List your products</button>
							...
							<div class="pt-20 pb-40 text-left radius20 holder">
								...
								<div class="mt-40 row flex-column-reverse flex-sm-row align-items-center buttons">
									...
									<div class="col-md-8 col-sm-9 block">
                                        <button onClick={(e)=> addProduct()}
                                        class="w-full d-flex align-items-center justify-content-center btn action-1 px-0"
                                        >
                                            <>
                                                <span>
                                                    {productLoading ? 'Loading..' : 'List product' }
                                                </span>
                                                {!productLoading && <i class="fas fa-chevron-right f-12 ml-5" />}
                                            </>
										</button>
									</div>
								</div>
